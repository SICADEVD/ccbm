...
class FormationAdapter(private var acti: Activity ,private var formations: List<FormationModel>?) : RecyclerView.Adapter<FormationHolder>() {
    ...
    override fun onBindViewHolder(holder: FormationHolder, position: Int) {
        ...
        val listTypeFormation = ProgBandRoomDatabase.getDatabase(acti)?.typeFormationDao()?.getAll(
            SPUtils.getInstance().getInt(Constants.AGENT_ID).toString())
        val listThemeFormation = ProgBandRoomDatabase.getDatabase(acti)?.themeFormationDao()?.getAll(
            SPUtils.getInstance().getInt(Constants.AGENT_ID).toString())
        val listSousThemeFormation = ProgBandRoomDatabase.getDatabase(acti)?.sousThemeFormationDao()?.getAll(
            SPUtils.getInstance().getInt(Constants.AGENT_ID).toString())
