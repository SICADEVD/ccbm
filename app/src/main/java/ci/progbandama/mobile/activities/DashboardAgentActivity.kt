...
class DashboardAgentActivity : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        agentDao = progbandRoomDatabase?.agentDoa();
        val coopDao = progbandRoomDatabase?.coopDao();
        ...
        imgProfileDashboard.setOnClickListener {
            ...
            builder.setPositiveButton(getString(R.string.oui)) { dialog, _ ->
                ...
                ProgBandRoomDatabase.getDatabase(this)?.agentDoa()
...
//         Initialize the Update Manager with the Activity and the Update Mode
    ...
    private fun setupLiveData() {
        ...
        ProgBandRoomDatabase.getDatabase(this)?.infosProducteurDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
        ...
        ProgBandRoomDatabase.getDatabase(this)?.suiviApplicationDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "APPLICATION", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.inspectionDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "INSPECTION", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.estimationDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "ESTIMATION", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.suiviParcelleDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "PARCELLES", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.formationDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "FORMATION", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.enqueteSsrtDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "SSRTECLMRS", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.livraisonDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "LIVRAISON", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.evaluationArbreDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "AGRO_EVALUATION", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.distributionArbreDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "AGRO_DISTRIBUTION", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.postplantingDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "POSTPLANTING", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.livraisonCentralDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "LIVRAISON_MAGCENTRAL", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.visiteurFormationDao()?.getUnSyncedAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.observe(this, Observer { items ->
            val length = items.size
            updateRVFeatureCount(lisFeat, "FORMATION_VISITEUR", length)
        })

        ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.getAllLive(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString())?.observe(this, Observer { items ->
...
//        data.add(Data(3f, ProgBandRoomDatabase.getDatabase(this)?.producteurMenageDoa()?.getSyncedAll(SPUtils.getInstance().getInt(Constants.AGENT_ID).toString())?.count()?.toFloat()?:0.0f, "MENAGE", Color.GRAY))
//        data.add(Data(4f, ProgBandRoomDatabase.getDatabase(this)?.evaluationArbreDao()?.getSyncedAll(SPUtils.getInstance().getInt(Constants.AGENT_ID).toString())?.count()?.toFloat()?:0.0f, "EVALUATION", Color.GREEN))
//
...
//        dataUnSync.add(Data(3f, ProgBandRoomDatabase.getDatabase(this)?.producteurMenageDoa()?.getUnSyncedAll(SPUtils.getInstance().getInt(Constants.AGENT_ID).toString())?.count()?.toFloat()?:0.0f, "MENAGE", Color.GRAY))
//        dataUnSync.add(Data(4f, ProgBandRoomDatabase.getDatabase(this)?.evaluationArbreDao()?.getUnSyncedAll(SPUtils.getInstance().getInt(Constants.AGENT_ID))?.count()?.toFloat()?:0.0f, "EVALUATION", Color.GREEN))
//
...
//        dataDraft.add(Data(0f, ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "producteur")?.toFloat()?:0.0f, "PRODUCTEUR", Color.BLACK))
//        dataDraft.add(Data(1f, ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "parcelle")?.toFloat()?:0.0f, "PARCELLE", Color.BLUE))
//        dataDraft.add(Data(2f, ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "formation")?.toFloat()?:0.0f, "FORMATION", Color.MAGENTA))
//        dataDraft.add(Data(3f, ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "menage")?.toFloat()?:0.0f, "MENAGE", Color.GRAY))
//        dataDraft.add(Data(4f, ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "evaluation_arbre")?.toFloat()?:0.0f, "EVALUATION", Color.GREEN))
//
...
//                    linearUniteAgricole.visibility = View.VISIBLE
                    ...
                    listOfFeatures.add(FeatureModel("PRODUCTEUR",
                        ...
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "producteur")!!,
                        type = "PRODUCTEUR",
                    ...
                    listOfFeatures.add(FeatureModel("INFOS PRODUCTEUR",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.infosProducteurDao()
                            ?.getUnSyncedAll(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "infos_producteur")!!,
                ...
                "PARCELLE" -> {
                    ...
                    listOfFeatures.add(FeatureModel("IDENTIFICATIONS PARCELLES",
                        ...
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "parcelle")!!,
                        type = "PARCELLE",
                        categorie = 0,
                ...
                "MENAGES","MENAGE" -> {
                    ...
                    listOfFeatures.add(FeatureModel("ENQUÊTE MÉNAGE",
                        ...
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "menage")!!,
                        type = "MENAGE",
                        categorie = 3,
                ...
                "SUIVIAPPLICATIONS","APPLICATION" -> {
                    ...
                    listOfFeatures.add(FeatureModel("APPLICATIONS PHYTOS",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.suiviApplicationDao()?.getUnSyncedAll()?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "application")!!,
                        type = "APPLICATION",
                        categorie = 1,
                ...
                "EVALUATIONS","INSPECTION" -> {
                    ...
                    listOfFeatures.add(FeatureModel("INSPECTIONS",
                        countSync =  ProgBandRoomDatabase.getDatabase(this)?.inspectionDao()?.getUnSyncedAll(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "inspection")!!,
                        type = "INSPECTION",
                ...
                "ESTIMATIONS","ESTIMATION" -> {
                    ...
                    listOfFeatures.add(FeatureModel("ESTIMATIONS",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.estimationDao()?.getUnSyncedAll()?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "estimation")!!,
                        type = "ESTIMATION",
                        categorie = 0,
                ...
                "SUIVIPARCELLES","PARCELLES" -> {
                    ...
                    listOfFeatures.add(FeatureModel("SUIVIS PARCELLES",
                        ...
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "suivi_parcelle")!!,
                        type = "PARCELLES",
                        categorie = 1,
                ...
                "SUIVIFORMATION","FORMATION" -> {
                    ...
                    listOfFeatures.add(FeatureModel("FORMATIONS",
                        ...
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "formation")!!,
                        type = "FORMATION",
                        categorie = 1,
                ...
                "SSRTECLMRS","SSRTECLMR" -> {
                    ...
                    listOfFeatures.add(FeatureModel("SSRTE-CLMRS",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.enqueteSsrtDao()?.getUnSyncedAll()?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "ssrte")!!,
                        type = "SSRTECLMRS",
                        categorie = 3,
                ...
                "LIVRAISONS","LIVRAISON" -> {
                    ...
                    listOfFeatures.add(FeatureModel("STOCK MAGASINS_SECTION",
                        ...
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "livraison")!!,
                        type = "LIVRAISON",
                        categorie = 2,
                ...
                "AGRO_EVALUATION" -> {
                    listOfFeatures.add(FeatureModel("EVALUATION BESOINS",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.evaluationArbreDao()?.getUnSyncedAll(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0))?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "AGRO_EVALUATION".lowercase())!!,
                ...
                "AGRO_DISTRIBUTION" -> {
                    listOfFeatures.add(FeatureModel("DISTRIBUTION D'ARBRE",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.distributionArbreDao()?.getUnSyncedAll(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "AGRO_DISTRIBUTION".lowercase())!!,
                ...
                "POSTPLANTING" -> {
                    listOfFeatures.add(FeatureModel("EVALUATION POST-PLANTING",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.postplantingDao()?.getUnSyncedAll(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "postplanting")!!,
                        type = "POSTPLANTING",
                        categorie = 4,
                        //image = R.drawable.livraison,
                        icon = R.drawable.distrib_arbre,
                ...
                "LIVRAISON_MAGCENTRAL" -> {
                    listOfFeatures.add(FeatureModel("STOCK MAGASINS_CENTRAUX",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.livraisonCentralDao()?.getUnSyncedAll(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString())?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "suivi_livraison_central")!!,
                ...
                "FORMATION_VISITEUR" -> {
                    listOfFeatures.add(FeatureModel("VISITEUR FORMATION",
                        countSync = ProgBandRoomDatabase.getDatabase(this)?.visiteurFormationDao()?.getUnSyncedAll(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toInt())?.size!!,
                        countDraft = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.countByType(agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID).toString(), type = "visiteur_formation")!!,
