...
//        }
    ...
    fun setupMagSectSelection(currVal: String? = null, currVal2: String? = null){
        ...
        val magasinSecList = ProgBandRoomDatabase.getDatabase(applicationContext)?.magasinSectionDao()
            ?.getAll()
    ...
    fun setupMagCentralSelection(currVal: String? = null, currVal2: String? = null){
        ...
        val magasinCentralList = ProgBandRoomDatabase.getDatabase(applicationContext)?.magasinCentralDao()
            ?.getAll()
    ...
    fun setAllListener() {
        ...
        Commons.setupItemMultiSelection(this, binding.selectTypProduitLivraisonCentral,
            ...
            it.forEach {produit ->
                ...
                livraisonVerMagCentralModelList.addAll(ProgBandRoomDatabase.getDatabase(this)?.livraisonVerMagCentralDao()?.getLivraisonByTypeProdAndMagSection(produit, magasinSectionCommon.nom)?.toMutableList()?: arrayListOf())
            }
...
//        }
    ...
    fun setupTransporteurSelection(currVal: String? = null, entrId: String? = "0") {
        val transporteurList = ProgBandRoomDatabase.getDatabase(applicationContext)?.transporteurDao()
    ...
    fun undraftedDatas(draftedData: DataDraftedModel) {
        ...
        Commons.setListenerForSpinner(this,
            ...
            onChanged = {
                entrepriseCommon.nom = entrepList!![it].nom
                entrepriseCommon.id = entrepList[it].id
                setupTransporteurSelection(ProgBandRoomDatabase.getDatabase(this)?.transporteurDao()?.getById(livraisonCentralDrafted?.sender_transporteur)?.let {
                    "${it.nom} ${it.prenoms}"
...
//        LogUtils.d(livraisonCentralDrafted?.sender_transporteur)
        setupTransporteurSelection(
            ProgBandRoomDatabase.getDatabase(this)?.transporteurDao()?.getById(livraisonCentralDrafted?.sender_transporteur)?.let {
                "${it.nom} ${it.prenoms}"
            }
...
//            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.getStringExtra("from") != null) {
            draftedDataLivraisonCentral = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.getDraftedDataByID(intent.getIntExtra("drafted_uid", 0)) ?: DataDraftedModel(uid = 0)
