...
import ci.progbandama.mobile.tools.Commons.Companion.convertDate
import ci.progbandama.mobile.tools.Commons.Companion.showMessage
import ci.progbandama.mobile.tools.Commons.Companion.toCheckEmptyItem
import ci.progbandama.mobile.tools.Commons.Companion.toModifString
import ci.progbandama.mobile.tools.Constants
import ci.progbandama.mobile.tools.LoadProgressListener
...
class InspectionActivity : AppCompatActivity(), SectionCallback,
    ...
    fun setupProducteurSelection(id: Int, currVal2: String? = null, currVal3: String? = null, currVal4: String? = null) {
        val producteursList = ProgBandRoomDatabase.getDatabase(applicationContext)?.producteurDoa()
            ...
            onChanged = {
                        ...
                        producteursList?.let { list ->
                            ...
                            if (producteur.isSynced) {
                                ...
                                listCertif = ProgBandRoomDatabase.getDatabase(applicationContext)?.producteurDoa()?.getProducteur(producteurID = producteur.id)
                            } else {
                                producteurId = producteur.uid.toString()
                                producteurCommon.nom = "${producteur.nom!!} ${producteur.prenoms!!}"
                                producteurCommon.id = producteur.uid
                                listCertif = ProgBandRoomDatabase.getDatabase(applicationContext)?.producteurDoa()?.getProducteurByUID(producteurUID = producteur.uid)
                            }
...
//        LogUtils.json(parcellesList)
    ...
    fun fetchQuestionnairesReview(fromDraft: Boolean, fromCertificat: String) {
        ...
        if (!fromDraft) {
            cQuestionnaires = ProgBandRoomDatabase.getDatabase(this)?.questionnaireDao()?.getAll()
...
//                    LogUtils.d(questionResponseInfoModel)
    ...
    private fun initRvList(cQuestionnairesReviewList: MutableList<QuestionResponseModel>?) {
        ...
        notationsList.addAll(
            ...
            //ProgBandRoomDatabase.getDatabase(this)?.notationDao()?.getAll()!!
        )
...
//        })
    ...
    fun setupEncareurSelection(currVal: String? = null){
        ...
        val encadreurList = ProgBandRoomDatabase.getDatabase(applicationContext)?.staffFormation()?.getAll( agentID = SPUtils.getInstance().getInt(Constants.AGENT_ID, 0).toString() )
...
//            firstBarprogress.showProgressText(true)
    ...
    private fun getInspectObjet(isMissingDial:Boolean = true, necessaryItem: MutableList<String> = arrayListOf(), id:Int = 0):  Pair<InspectionDTO, MutableList<Pair<String, String>>>? {
        ...
        if(id != 0){
            inpectMod = ProgBandRoomDatabase.getDatabase(this@InspectionActivity)?.inspectionDao()?.getByUid(id)?.first()!!
        }
...
//                LogUtils.e("From draft")
                ...
                draftedDataInspection = ProgBandRoomDatabase.getDatabase(this)?.draftedDatasDao()?.getDraftedDataByID(intent.getIntExtra("drafted_uid", 0)) ?: DataDraftedModel(uid = 0)
...
//        checkCommentField()
    ...
    private fun lauchForUpdate(inspectionDrafted: InspectionDTO) {
        ...
        val product = ProgBandRoomDatabase.getDatabase(this)?.producteurDoa()?.getProducteurByID(inspectionDrafted.producteursId?.toInt()?:0)
